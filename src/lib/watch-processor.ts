...
async function runBinding(binding: Binding, capabilityNamespaces: string[]) {
  ...
  const watchCallback = async (obj: KubernetesObject, type: WatchPhase) => {
    ...
    if (phaseMatch.includes(type)) {
      try {
        ...
        Log.error(e, `${cacheId}: Error executing watch callback`);
      }
    }
  };
  ...
  const watcher = K8s(binding.model, binding.filters).Watch(async (obj, type) => {
    Log.debug(obj, `${cacheId}: Watch event ${type} received`);
  ...
  cacheId = watcher.getCacheID();

  queue.setCacheId(cacheId);
  queue.setReconcile(watchCallback);

  Log.debug({ watchCfg }, `${cacheId}: Effective WatchConfig`);

  // If failure continues, log and exit
  watcher.events.on(WatchEvent.GIVE_UP, err => {
    Log.error(err, `${cacheId}: Watch failed after 5 attempts, giving up`);
    process.exit(1);
  });
